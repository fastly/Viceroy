use {
    crate::component::fastly::compute::{purge, types},
...
impl purge::Host for ComponentCtx {
    ...
    async fn purge_surrogate_key_verbose(
        &mut self,
...
impl purge::HostExtraPurgeOptions for ComponentCtx {
    async fn drop(&mut self, _options: Resource<purge::ExtraPurgeOptions>) -> wasmtime::Result<()> {
        Ok(())
    }
}

...
use crate::component::fastly::compute::{http_body, http_downstream, http_req, types};
...
impl http_downstream::Host for ComponentCtx {
    ...
    async fn await_next_request(
        &mut self,
        handle: Resource<http_downstream::RequestPromise>,
    ) -> Result<(Resource<http_req::Request>, Resource<http_body::Body>), types::Error> {
        let handle = RequestPromiseHandle::from(handle).into();
    ...
    async fn downstream_client_h2_fingerprint(
        ...
        Ok(self.session().absent_metadata_value(h.into())?)
    }
    ...
    async fn downstream_compliance_region(
        ...
        match u64::try_from(region_len) {
            Ok(region_len) if region_len <= region_max_len => Ok(region.to_owned()),

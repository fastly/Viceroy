...
viceroy_test!(kv_store_bad_configs, |is_component| {
    ...
    // Invalid format JSON - entry must have data or path
    ...
    match Test::using_fixture("kv_store.wasm").using_fastly_toml(BAD_16_FASTLY_TOML) {
        Err(e) => assert_eq!("invalid configuration for 'store_one': Item value under key named 'first' is of the wrong format. One of 'data' and 'path' must be present.", &e.to_string()),
        _ => panic!(),
    }

    // Invalid format JSON - entry cannot have both data and path
    ...
    match Test::using_fixture("kv_store.wasm").using_fastly_toml(BAD_17_FASTLY_TOML) {
        Err(e) => assert_eq!("invalid configuration for 'store_one': Item value under key named 'first' is of the wrong format. 'data' and 'path' are mutually exclusive.", &e.to_string()),
        _ => panic!(),
    }

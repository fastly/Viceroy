use {
    crate::component::fastly::compute::{config_store, types},
...
impl config_store::HostStore for ComponentCtx {
    async fn open(&mut self, name: String) -> Result<Resource<config_store::Store>, types::Error> {
    ...
    async fn drop(&mut self, _store: Resource<config_store::Store>) -> wasmtime::Result<()> {
        Ok(())
    }
}

use crate::component::fastly::compute::{acl, http_body, types};
...
impl acl::HostAcl for ComponentCtx {
    async fn open(&mut self, acl_name: String) -> Result<Resource<acl::Acl>, types::Error> {
        let handle = self
            .session_mut()
    ...
    ) -> Result<(Option<Resource<http_body::Body>>, acl::AclError), types::Error> {
        let acl = self
            .session()
    ...
    async fn drop(&mut self, _acl_handle: Resource<acl::Acl>) -> wasmtime::Result<()> {
        Ok(())
    }
}

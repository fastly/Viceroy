use {
    crate::component::fastly::compute::async_io,
    crate::{
...
impl async_io::Host for ComponentCtx {
    ...
    async fn select_with_timeout(
        ...
        let res = tokio::time::timeout(Duration::from_millis(timeout_ms as u64), select_fut)
            .await
            .ok()
...
impl async_io::HostPollable for ComponentCtx {
    async fn new_ready(&mut self) -> Resource<async_io::Pollable> {
        wiggle_abi::types::AsyncItemHandle::from(self.session_mut().new_ready()).into()
    }

    async fn is_ready(&mut self, handle: Resource<async_io::Pollable>) -> bool {
    ...
    async fn drop(&mut self, _pollable: Resource<async_io::Pollable>) -> wasmtime::Result<()> {
        Ok(())
    }
}

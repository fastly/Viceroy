...
pub enum ObjectStoreConfigError {
    ...
    #[error("Item value under key named '{key}' is of the wrong format. 'data' and 'path' are mutually exclusive.")]
    BothDataAndPathPresent { key: String },
    #[error("Item value under key named '{key}' is of the wrong format. One of 'data' and 'path' must be present.")]
    MissingDataOrPath { key: String },
}

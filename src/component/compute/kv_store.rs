use {
    crate::component::fastly::compute::{
        http_body,
...
impl kv_store::HostEntry for ComponentCtx {
    ...
    async fn generation(&mut self, rep: wasmtime::component::Resource<kv_store::Entry>) -> u64 {
...
impl kv_store::HostStore for ComponentCtx {
    async fn open(
        &mut self,
        name: String,
    ) -> Result<Option<Resource<kv_store::Store>>, types::Error> {
    ...
    async fn lookup_async(
        ...
        let fut = futures::future::ok(
            self.session()
                .obj_lookup(store.clone(), ObjectKey::new(key)?),
        );
    ...
    async fn insert_async(
        ...
        let meta = if let Some(metadata) = options.metadata {
            Some(metadata)
        } else {
            None
        };

        let igm = if let Some(if_generation_match) = options.if_generation_match {
            Some(if_generation_match)
        } else {
            None
        };
    ...
    async fn delete_async(
        ...
        let fut = futures::future::ok(
            self.session()
                .kv_delete(store.clone(), ObjectKey::new(key)?),
        );
        let task = PeekableTask::spawn(fut).await;
        let lh = self
            .session_mut()
    ...
    async fn drop(&mut self, _store: Resource<kv_store::Store>) -> wasmtime::Result<()> {
        Ok(())
    }
}
...
impl kv_store::HostExtraInsertOptions for ComponentCtx {
    async fn drop(
        &mut self,
        _options: Resource<kv_store::ExtraInsertOptions>,
    ) -> wasmtime::Result<()> {
        Ok(())
    }
}
...
impl kv_store::HostExtraListOptions for ComponentCtx {
    async fn drop(
        &mut self,
        _options: Resource<kv_store::ExtraListOptions>,
    ) -> wasmtime::Result<()> {
        Ok(())
    }
}

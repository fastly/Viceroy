        ...
        if consumer.cursor >= *offset {
            ...
            if let Some(result) = buffer.get(buffer_index) {
                ...
                return Poll::Ready(Some(
                    result.clone().map_err(|e| Box::new(e) as Self::Error),
                ));
            }
        }
...
mod tests {
    ...
    async fn test_simple_duplication() {
        ...
        let res1_str: Vec<&str> = res1.iter().map(|b| std::str::from_utf8(b).unwrap()).collect();
        let res2_str: Vec<&str> = res2.iter().map(|b| std::str::from_utf8(b).unwrap()).collect();
    ...
    async fn test_error_propagation() {
        ...
        assert!(body1.next().await.is_none(), "Stream should end after error");
        ...
        assert!(body2.next().await.is_none(), "Stream should end after error");
    }
    ...
    async fn test_error_with_one_consumer_dropped() {
        ...
        let err1 = body1.next().await.unwrap().unwrap_err();
        assert_eq!(err1.to_string(), "aborted");
        assert!(body1.next().await.is_none(), "Stream should end after error");
    }
    ...
    async fn test_size_hint_preservation() {
        ...
        assert_eq!(HttpBody::size_hint(&body1).exact(), original_size_hint.exact());
        assert_eq!(HttpBody::size_hint(&body2).exact(), original_size_hint.exact());

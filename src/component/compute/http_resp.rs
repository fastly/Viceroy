use {
    ...
    crate::component::fastly::compute::{http_body, http_resp, http_types, types},
    crate::{
        error::Error,
...
impl http_resp::Host for ComponentCtx {
    ...
    async fn close(&mut self, h: Resource<http_resp::Response>) -> Result<(), types::Error> {
...
impl http_resp::HostResponse for ComponentCtx {
    async fn new(&mut self) -> Result<Resource<http_resp::Response>, types::Error> {
    ...
    async fn set_http_keepalive_mode(
        &mut self,
    ...
    async fn get_remote_port(&mut self, resp_handle: Resource<http_resp::Response>) -> Option<u16> {
        let resp = self.session().response_parts(resp_handle.into()).unwrap();
        let md = resp.extensions.get::<upstream::ConnMetadata>()?;
    ...
    async fn drop(&mut self, _response: Resource<http_resp::Response>) -> wasmtime::Result<()> {
        Ok(())
    }
}

...
impl adapter_http_req::Host for ComponentCtx {
    ...
    async fn downstream_tls_ja4(&mut self, max_len: u64) -> Result<String, types::Error> {
    ...
    async fn downstream_client_ip_addr(&mut self) -> Option<types::IpAddress> {
        let h = self.ds_req_handle();
        HttpDownstream::downstream_client_ip_addr(self, h).await
    }

    async fn downstream_server_ip_addr(&mut self) -> Option<types::IpAddress> {
        let h = self.ds_req_handle();

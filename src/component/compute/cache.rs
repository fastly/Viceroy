use {
    crate::component::fastly::compute::{cache as api, http_body, types},
    crate::{
        body::Body,
...
impl api::Host for ComponentCtx {
    ...
    async fn close(&mut self, handle: Resource<api::Entry>) -> Result<(), types::Error> {
        let _ = self
            .session_mut()
...
impl api::HostEntry for ComponentCtx {
    ...
    async fn get_body(
        ...
        let to = if let Some(to) = options.to {
            Some(to)
        } else {
            None
        };
        ...
        let body_handle = body_handle.into();
    ...
    async fn get_length(&mut self, handle: Resource<api::Entry>) -> Result<u64, types::Error> {
        let found = self
            .session_mut()
    ...
    async fn get_max_age_ns(&mut self, handle: Resource<api::Entry>) -> Result<u64, types::Error> {
        let entry = self.session_mut().cache_entry_mut(handle.into()).await?;
        if let Some(found) = entry.found() {
    ...
    async fn get_age_ns(&mut self, handle: Resource<api::Entry>) -> Result<u64, types::Error> {
        let entry = self.session_mut().cache_entry_mut(handle.into()).await?;
        if let Some(found) = entry.found() {
    ...
    async fn get_hits(&mut self, _handle: Resource<api::Entry>) -> Result<u64, types::Error> {
        Err(Error::Unsupported {
            msg: "Cache API primitives not yet supported",
        }
        .into())
    ...
    async fn drop(&mut self, _entry: Resource<api::Entry>) -> wasmtime::Result<()> {
        Ok(())
    }
}

impl api::HostExtraReplaceOptions for ComponentCtx {
    async fn drop(&mut self, _options: Resource<api::ExtraReplaceOptions>) -> wasmtime::Result<()> {
        Ok(())
    }
}

impl api::HostExtraGetBodyOptions for ComponentCtx {
    async fn drop(&mut self, _options: Resource<api::ExtraGetBodyOptions>) -> wasmtime::Result<()> {
        Ok(())
    }
}

impl api::HostExtraWriteOptions for ComponentCtx {
    async fn drop(&mut self, _options: Resource<api::ExtraWriteOptions>) -> wasmtime::Result<()> {
        Ok(())
    }
}

impl api::HostExtraLookupOptions for ComponentCtx {
    async fn drop(&mut self, _options: Resource<api::ExtraLookupOptions>) -> wasmtime::Result<()> {
        Ok(())
    }
}

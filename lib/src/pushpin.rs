...
pub async fn proxy_through_pushpin(
    ...
    let (path_and_query, method) = if let Some(ref info) = redirect_request_info {
        (info.path_and_query.as_deref().unwrap_or(""), info.method.as_str())
    } else {
        (original_request_info.path_and_query.as_deref().unwrap_or(""), original_request_info.method.as_str())
    };
    ...
    if let Some(redirect_request_info) = redirect_request_info {
        ...
        for (name, value) in &original_request_info.headers {
            if PROTECTED_REQ_HEADERS.iter().any(|h| h.eq_ignore_ascii_case(name.as_str())) {
    ...
    if upstream_resp.status() == StatusCode::SWITCHING_PROTOCOLS {
        ...
        tokio::spawn(proxy_upgraded_connection(original_request_on_upgrade, conn_fut));
    }

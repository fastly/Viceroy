use {
    crate::component::{
        compute::headers,
        fastly::compute::{http_body, types},
    },
    crate::{
        body::Body,
        error::Error,
...
impl http_body::Host for ComponentCtx {
    async fn new(&mut self) -> Result<Resource<http_body::Body>, types::Error> {
    ...
    async fn close(&mut self, h: Resource<http_body::Body>) -> Result<(), types::Error> {
    ...
    async fn get_known_length(&mut self, h: Resource<http_body::Body>) -> Option<u64> {
        let h = h.into();
        if self.session().is_streaming_body(h) {
            None
        } else if let Some(len) = self.session_mut().body_mut(h).unwrap().len() {
            Some(len)
        } else {
            None
        }

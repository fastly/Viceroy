use {
    crate::component::fastly::compute::{secret_store, types},
    crate::{
        error::Error,
...
impl secret_store::HostStore for ComponentCtx {
    async fn open(&mut self, name: String) -> Result<Resource<secret_store::Store>, types::Error> {
        let handle =
            self.session_mut()
    ...
    async fn drop(&mut self, _store: Resource<secret_store::Store>) -> wasmtime::Result<()> {
        Ok(())
    }
}
...
impl secret_store::HostSecret for ComponentCtx {
    ...
    async fn drop(&mut self, _secret: Resource<secret_store::Secret>) -> wasmtime::Result<()> {
        Ok(())
    }
}

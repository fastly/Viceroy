...
impl ExecuteCtx {
   ...
   +------------------------------------------------------------------------+
    ...
    pub async fn handle_request(
        ...
        let orig_request_info_for_pushpin = PushpinRedirectRequestInfo::from_parts(&incoming_req_parts);
        ...
        let (resp, mut err) = self.spawn_guest(req, metadata).await;
                ...
                Ok(NonHttpResponse::PushpinRedirect(redirect_info)) => {
                    ...
                    let local_pushpin_proxy_port = match local_pushpin_proxy_port {
                        None => {
                            ...
                            let err = anyhow::anyhow!("Pushpin redirect signaled, but Pushpin mode not enabled.");
    ...
    async fn spawn_guest(
        ...
        if let Ok(resp) = receiver.await {
            return match resp {
                ...
                DownstreamResponse::RedirectToPushpin(info) => {
                    (Response::new(Body::empty()), Some(NonHttpResponse::PushpinRedirect(info).into()))
                }
            }
        }
